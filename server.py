...
catalog = "quaranteen" #базовый каталог карантина
...
filename - название файла
pattern - набор байтов в виде hex строки, он же искомая сигнатура
...
def sigfinder(filename,pattern):
    ...
    print(res1[:-1]) #отрезаем запятую в конце
    if len(res1)==0: #в случае отсуствия вхождений - возвращаем информацию о том, что сигнатура не обнаружина
        ...
        print(res1)
        return res1
    return res1[:-1] #возвращаем именно строку для удобства, но по сути это список, можно распарсить через re.sub
...
def handler(json_string):
    parsed_json = json.loads(json_string) #парсинг данных для дальнейшего разбиения, по сути переводим JSON объект назад в словарь и достаем значения по ключам
    print(parsed_json)
...
def recieve(threads):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor: #создание пула потоков, по мере поступления соединений создаются потоки, при отсуствии свободных - клиент ждет
        ...
        s.close()
'''
Обработчик сигнала SIGINT, по сути питон итак на него реагирует, но вроде как указано в ТЗ
'''
